...
static inline u16 prctl_option_to_scap(unsigned long option)
{
	switch (option) {
	case PR_SET_CHILD_SUBREAPER:
		return PPM_PRCTL_SET_CHILD_SUBREAPER;
	case PR_GET_CHILD_SUBREAPER:
		return PPM_PRCTL_GET_CHILD_SUBREAPER;
	default:
		return PPM_PRCTL_UNKNOWN;
	}
}
...
static int f_sys_prctl_x(struct event_filler_arguments *args)
{
	 ...
	 */
	syscall_get_arguments(current, args->regs, 0, 1, &val);
	res = val_to_ring(args, (unsigned long)prctl_option_to_scap(val), 0, false, 0);
	if (unlikely(res != PPM_SUCCESS))
		return res;

...
#ifdef HAS_CAPTURE
static void sigchld_callback(int signal)
{
	...
	if (g_exec_pgid > 0)
	{
		if (kill(-g_exec_pgid, 0) < 0 && errno == ESRCH)
		{
			g_drain = true;
		}
	}
		...
		if (exec_command) {
			...
			switch(command_pid) {
				case 0: // child
					{
						...
						if(setpgid(0, 0) < 0)
						{
							perror("Failed to create process group");
							_exit(255);
						}
				...
				case -1: // fork error
					...
					goto exit;
				default:
					string pid_filter = "proc.pgid = " + to_string(command_pid);
					if (filter.size())
					{

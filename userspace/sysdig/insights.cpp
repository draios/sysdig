...
insights_runner::insights_runner(sinsp* inspector)
{
	...
	add(insight_info("EC2 instance run from suspicious country", "ct.name=RunInstances and ct.user.country!=us", "new EC2 instance started by a user in a suspicious country", insight_info::SEV_HIGH, {"ct.user.country", "ct.user", "ct.region", "ct.srcip", "ct.status"}));
	...
	add(insight_info("console login from a suspicious country", "ct.name = ConsoleLogin and ct.user.country!=us", "a user logged in to the console from a suspicious country", insight_info::SEV_HIGH, {"ct.user.country", "ct.user", "ct.region", "ct.srcip", "ct.status"}));
	...
	add(insight_info("EC2 KeyPair operation", "ct.shortsrc = ec2 and ct.readonly = false and ct.name contains KeyPair", "Stored EC2 KeyPairs were modified", insight_info::SEV_MEDIUM, {"ct.user", "ct.srcip", "ct.status"}));
	...
	add(insight_info("failed infrastructure modifying event", "ct.error exists and ct.readonly=false", "actions that modify the state of the AWS environment but failed", insight_info::SEV_INFO, {"ct.name", "ct.user", "ct.region", "ct.srcip"}));

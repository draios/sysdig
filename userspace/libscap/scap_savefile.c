...
int32_t scap_write_proclist_entry(scap_t *handle, scap_dumper_t *d, struct scap_threadinfo *tinfo)
{
	...
	ttynamelen = (uint16_t)strnlen(tinfo->ttyname, SCAP_MAX_PATH_SIZE);
	...
	if(scap_dump_write(d, &(tinfo->tid), sizeof(uint64_t)) != sizeof(uint64_t) ||
			...
			scap_dump_write(d, &ttynamelen, sizeof(uint16_t)) != sizeof(uint16_t) ||
			scap_dump_write(d, tinfo->ttyname, ttynamelen) != ttynamelen)
	{
...
static int32_t scap_read_proclist(scap_t *handle, gzFile f, uint32_t block_length, uint32_t block_type)
{
	...
	while(((int32_t)block_length - (int32_t)totreadsize) >= 4)
	{
		...
		case PL_BLOCK_TYPE_V8:
			...
			if(stlen > SCAP_MAX_PATH_SIZE)
			{
				snprintf(handle->m_lasterr, SCAP_LASTERR_SIZE, "invalid ttynamelen %d", stlen);
				return SCAP_FAILURE;
			}

...
bool sinsp_container_manager::resolve_container(sinsp_threadinfo* tinfo, bool query_os_for_missing_info)
{
	...
	for(auto it = tinfo->m_cgroups.begin(); it != tinfo->m_cgroups.end(); ++it)
	{
        ...
        pos = cgroup.find_last_of("/");
        if ((pos != string::npos) && (cgroup.size() - pos) > 1)
        {
            // Example of container ID generated by garden-runc (TODO: its not standard
            // uuid format, need to check the source to figure what's being used):
            // "/7c1d0605-3303-47e2-7ada-c694"
            const string ct_id = cgroup.substr(pos + 1, cgroup.size() - pos);
            if (!lookup_garden_runtime(ct_id))
            {
                break;
            }

            // Don't let resolve_container() succeed for runc's init process cause it
            // does not yield app specific metadata.
            if ((tinfo->m_comm.find("runc") != string::npos) &&
                (tinfo->m_comm.find("INIT") != string::npos))
            {
                return false;
            }
            container_info.m_type = CT_GARDEN;
            container_info.m_id = ct_id;
            valid_id = true;
            break;
        }
	}
		...
		if(it == m_containers.end())
		{
			switch(container_info.m_type)
			{
				...
				case CT_RKT:
					...
					break;
                case CT_GARDEN:
                    parse_garden(&container_info, tinfo);
                    break;
				default:
					ASSERT(false);
			}
...
bool sinsp_container_manager::parse_garden(sinsp_container_info* container,
                                           sinsp_threadinfo* tinfo)
{
    ...
    string vcap_app_env;
    string cf_instance_index_env;
    string cf_instance_guid_env;
    string cf_instance_ip_env;
    string cf_instance_ports_env;
    string memory_limit_env;

    sinsp_threadinfo::visitor_func_t visitor =
        [ &vcap_app_env,
          &cf_instance_index_env,
          &cf_instance_guid_env,
          &cf_instance_ip_env,
          &cf_instance_ports_env,
          &memory_limit_env ] (sinsp_threadinfo *ptinfo)
    {
        vcap_app_env = ptinfo->get_env("VCAP_APPLICATION");
        if (!vcap_app_env.empty()) {
            cf_instance_index_env = ptinfo->get_env("CF_INSTANCE_INDEX");
            cf_instance_guid_env = ptinfo->get_env("CF_INSTANCE_GUID");
            cf_instance_ip_env = ptinfo->get_env("CF_INSTANCE_IP");
            cf_instance_ports_env = ptinfo->get_env("CF_INSTANCE_PORTS");
            memory_limit_env = ptinfo->get_env("MEMORY_LIMIT");
            return false;
        }
        return true;
    };
    ...
    if (!json_reader.parse(vcap_app_env, json_root)) {
        return false;
    }

    if (!cf_instance_guid_env.empty()) {
        if (container->m_id == cf_instance_guid_env) {
            // No new info to be gained. Set a name for the container.
            container->m_name = json_root["application_name"].asString() + ":" +
                                cf_instance_index_env;
        } else {
            // Use the cgroup name as the container name.
            container->m_name = container->m_id;
            container->m_id = cf_instance_guid_env;
        }
    }

    container->m_image = json_root["application_name"].asString();
    container->m_imageid = json_root["application_id"].asString();

    if (!cf_instance_ip_env.empty()) {
        if (inet_pton(AF_INET, cf_instance_ip_env.c_str(),
                      &container->m_container_ip) != -1) {
            container->m_container_ip = ntohl(container->m_container_ip);
        }
    }

    // container->m_container_ip must be set before this block
    if (!cf_instance_ports_env.empty()) {
        size_t pos_start = 0;
        size_t pos_end = 0;

        while ((pos_start = cf_instance_ports_env.find("{", pos_end))
                != string::npos &&
               (pos_end = cf_instance_ports_env.find("}", pos_start + 1))
                != string::npos) {
            string port_map_str = cf_instance_ports_env.substr(pos_start, pos_end);
            if (!json_reader.parse(port_map_str, json_root)) {
                continue;
            }

            sinsp_container_info::container_port_mapping port_map_ct;
            port_map_ct.m_host_ip = container->m_container_ip;
            port_map_ct.m_host_port = json_root["external"].asInt();
            port_map_ct.m_container_port = json_root["internal"].asInt();
            container->m_port_mappings.push_back(port_map_ct);
        }
    }

    if (!memory_limit_env.empty()) {
        const int unit = tolower(memory_limit_env.back());
        container->m_memory_limit = strtoll(memory_limit_env.c_str(), NULL, 0);

        switch (unit) {
            case 'g':
                container->m_memory_limit *= 1E9;
                break;
            case 'm':
                container->m_memory_limit *= 1E6;
                break;
            case 'k':
                container->m_memory_limit *= 1E3;
                break;
            default:
                break;
        }
    ...
    g_logger.format(sinsp_logger::SEV_DEBUG,
                        "%s: %s [%" PRId64 "]: "
                        "container_id %s, container_name %s, "
                        "image_id %s, image_name %s, "
                        "IP %s, ports %s, "
                        "memory_limit %" PRId64 "\n",
                        __func__, tinfo->m_comm.c_str(), tinfo->m_pid,
                        container->m_id.c_str(), container->m_name.c_str(),
                        container->m_imageid.c_str(), container->m_image.c_str(),
                        cf_instance_ip_env.c_str(), cf_instance_ports_env.c_str(),
                        container->m_memory_limit);
    return true;
}

bool sinsp_container_manager::lookup_garden_runtime(const string& container_id)
{
    ...
    for (auto const& it : paths) {
        abs_path = string(scap_get_host_root()) + it;
    ...
    strncpy(sock_addr.sun_path, abs_path.c_str(), sizeof(sock_addr.sun_path) - 1);
    ...
    const string query = "GET /containers/" + container_id + "/info HTTP/1.0\r\n\n";
    const string expect = "HTTP/1.0 200 OK";
    string response;
    if (write(sock, query.c_str(), query.length()) != (ssize_t)query.length()) {
        g_logger.format(sinsp_logger::SEV_DEBUG, "%s: write() failed: errno %d\n",
                        __func__, errno);
        close(sock);
        return false;
    }
    ...
    ssize_t read_size;
    unique_ptr<char[]> read_buf(new char[expect.length() + 1]());
    if (!read_buf) {
        close(sock);
        return false;
    }
    while ((read_size = read(sock, read_buf.get(), expect.length())) != 0) {
        if (read_size == -1) {
            g_logger.format(sinsp_logger::SEV_DEBUG, "%s: read() failed: errno %d\n",
                            __func__, errno);
            close(sock);
            return false;
        }

        response += read_buf.get();
        if (response.length() < expect.length()) {
            memset(read_buf.get(), 0, expect.length());
            continue;
        }

...
int32_t sinsp_filter_check_evtin::parse_field_name(const char* str, bool alloc_state)
{
	...
	else if(string(val, 0, sizeof("evtin.span.t.tag") - 1) == "evtin.span.t.tag" &&
		string(val, 0, sizeof("evtin.span.t.tags") - 1) != "evtin.span.t.tags")
	{
		m_field_id = TYPE_T_TAG;
		m_field = &m_info.m_fields[m_field_id];

		res = extract_arg("evtin.span.t.tag", val);
	}
	else if(string(val, 0, sizeof("evtin.span.t.arg") - 1) == "evtin.span.t.arg" &&
		string(val, 0, sizeof("evtin.span.t.args") - 1) != "evtin.span.t.args")
	{
		m_field_id = TYPE_T_ARG;
		m_field = &m_info.m_fields[m_field_id];

		res = extract_arg("evtin.span.t.arg", val);
	}
...
inline bool sinsp_filter_check_evtin::compare_tracer(sinsp_evt *evt, sinsp_partial_tracer* pae)
{
	...
	if(field_id >= TYPE_T_ID && field_id <= TYPE_T_ARG)
	{
		//

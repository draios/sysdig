...
# Fields
$BASEDIR/sysdig_batch_parser.sh $SYSDIG $CHISELS "-p*%fd.num %fd.type %fd.typechar %fd.name %fd.directory %fd.ip %fd.cip %fd.sip %fd.port %fd.cport %fd.sport %fd.l4proto %fd.sockfamily %fd.is_server" $TRACEDIR $RESULTDIR/fd_fields $BASELINEDIR/fd_fields
